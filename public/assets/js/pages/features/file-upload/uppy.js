...
var KTUppy = function () {
	...
	var initUppy2 = function(){
		...
		var options = {
			...
			replaceTargetContent: true,
			showProgressDetails: true,
			note: 'Images and documents only, 4-5 files. And window will be closed on files successfull upload.',
			height: 300,
			metaFields: [
				{ id: 'name', name: 'Name', placeholder: 'file name' },
				{ id: 'caption', name: 'Caption', placeholder: 'describe what the image is about' }
		...
		var uppyDashboard = Uppy.Core({
			...
			restrictions: {
				...
				maxNumberOfFiles: 5,
				minNumberOfFiles: 1,
				allowedFileTypes: ['image/*', 'application/pdf', '.doc', '.docx', '.xls', '.xlsx', '.csv']
			}
		});
		uppyDashboard.setMeta({ folder_id: document.querySelector('input[name="folder_id"]').value }) 
		uppyDashboard.use(Dashboard, options);
		//uppyDashboard.use(Tus, { endpoint: 'https://master.tus.io/files/' });
		var endPoint = document.querySelector('#uploadFilesForm').getAttribute('action');
		uppyDashboard.use(XHRUpload, {
            ...
            fieldName: 'file',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value // from <meta name="csrf-token" content="{{ csrf_token() }}">
            }
        });
        uppyDashboard.on('complete', function(files) {
			if(files.successful){
				var isChecked = $('#closeWindowCheck').is(':checked');
				if(isChecked){
					setTimeout(function(){ 
					    window.top.close();
					}, 1000);
